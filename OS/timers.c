...
BaseType_t xTimerCreateTimerTask(void)
{
    ...
    if (xTimerQueue != NULL)
    {
...
#if ( INCLUDE_xTimerGetTimerDaemonTaskHandle == 1 )
        {
            ...
            xReturn =
                xTaskGenericCreate(prvTimerTask, "Tmr Svc", usTimerTaskStackSize, NULL,
                                   ((UBaseType_t) configTIMER_TASK_PRIORITY) | portPRIVILEGE_BIT, &xTimerTaskHandle,
                                   pxTimerTaskStackBuffer, pxTimerTaskTCBBuffer, NULL);
        }
#else
        {
            ...
            xReturn =
                xTaskGenericCreate(prvTimerTask, "Tmr Svc", usTimerTaskStackSize, NULL,
                                   ((UBaseType_t) configTIMER_TASK_PRIORITY) | portPRIVILEGE_BIT, NULL,
                                   pxTimerTaskStackBuffer, pxTimerTaskTCBBuffer, NULL);
        }
#endif
    }
...
static void prvProcessExpiredTimer(const TickType_t xNextExpireTime, const TickType_t xTimeNow)
{
    ...
    if (pxTimer->uxAutoReload == (UBaseType_t) pdTRUE)
    {
        ...
        if (prvInsertTimerInActiveList
            (pxTimer, (xNextExpireTime + pxTimer->xTimerPeriodInTicks), xTimeNow, xNextExpireTime) != pdFALSE)
        {
    ...
    while (xQueueReceive(xTimerQueue, &xMessage, tmrNO_DELAY) != pdFAIL)    /*lint !e603 xMessage does not have to be initialised as it is passed out, not in, and it is not used unless xQueueReceive() returns pdTRUE. */
    {
        ...
        if (xMessage.xMessageID >= (BaseType_t) 0)
        {
            ...
            case tmrCOMMAND_START_DONT_TRACE:
                ...
                if (prvInsertTimerInActiveList
                    (pxTimer, xMessage.u.xTimerParameters.xMessageValue + pxTimer->xTimerPeriodInTicks, xTimeNow,
